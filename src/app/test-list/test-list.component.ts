...
export class TestListComponent implements OnInit {
    ...
    deleteTest(selectedTest: Test) {
        ...
        if(window.confirm('This action will delete business model \"' + selectedTest.Name + '\"". Are you sure?')) {
            this.dataService.deleteTest(selectedTest.Id).subscribe(resp => {
                if(resp.ok) {
                    alert("Test " + selectedTest.Name + " is successfully deleted!");
                    this.getTests();
                }
            })
        }
    ...
    addQuestion(selectedTestDTO: TestDTO) {
        this.messageService.selectTest(selectedTestDTO);
